            ...
            <Collapsible className="p-1 w-3/4" key={index}>
              ...
              <CollapsibleContent className="p-6 text-left w-full">
                <div className="flex flex-col gap-2">
                  ...
                  <h2 className="p-2 border rounded-lg bg-red-50 text-sm text-red-900"><strong>User Answer :</strong> {answer.userAnswer}</h2>
                  <h2 className="p-2 border rounded-lg bg-green-50 text-sm text-green-900"><strong>AI Answer :</strong> {answer.correctAns}</h2>
                  <h2 className="p-2 border rounded-lg bg-blue-50 text-sm text-blue-900"><strong>Feedback from AI :</strong> {answer.feedback}</h2>
                </div>
              </CollapsibleContent>
            </Collapsible>
          ))}
      </div>
      <Button className="self-start" onClick={() => router.replace("/main")}>Go to main page</Button>
    </div>
  );
}
